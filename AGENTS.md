...
- Config shape is `mysql.{dsn,detail_retain}`, `admin.{username,password}`, `api_keys[].{name,value}` and `models[].{name,providers[]}` with `providers[].{name,model,headers,openai_completions,openai_responses,anthropic_messages}`. Each vendor must set at least one protocol block `{url,model,headers}`. `url` is the full `http`/`https` upstream endpoint, not a base URL. Auth to upstream belongs in `headers`.
- `providers[].name` is the vendor: unique within a model, failover order, and `llm_calls.provider`. OpenAI chat/completions/embeddings routes only attempt providers that have an `openai_completions` block; `POST /v1/responses` only attempts those with an `openai_responses` block; `POST /v1/messages` only attempts those with an `anthropic_messages` block. Skip vendors missing that block; do **not** convert bodies or fail over to the other protocol. Missing matching providers → 404.
