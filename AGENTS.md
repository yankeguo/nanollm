...
| `templates/*.html` | Embedded HTML for the admin dashboard (`/usage`, `/calls`, `/login`). Bootstrap 5.3 (jsdelivr CDN, SRI) with `data-bs-theme="dark"` + Bootstrap Icons; hand-rolled CSS is limited to the token bars and code boxes |
...
- The admin dashboard lives at the root: `GET /` → 302 `/usage`; `/usage`, `/calls`, `/calls/{id}`, `/files/{sha}` are cookie-authenticated (HMAC, HttpOnly, SameSite=Lax; Secure when TLS or `X-Forwarded-Proto: https`), `/login` and `/logout` handle the session. It is not API-key auth. `admin.username` and `admin.password` are required.
