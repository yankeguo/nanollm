...
| `web/view/*.html` | Embedded HTML views for the admin dashboard (`/usage`, `/calls`, `/login`). Dark UI styled with Tailwind utilities plus the component classes from `main.css` (`.card`, `.btn*`, `.input`, `.tbl`, `.page-btn`, `.badge*`, token bars, `.codebox`); icons are `<span class="icon-[lucide--NAME]"></span>` (Iconify masks, colored by `currentColor`) |
| `admin_tmpl.go` | `//go:embed web/view/*.html`; template funcs `formatNum`, `inputBar`, `outputBar`, `statusClass`, `callErrorClass`, `jsAsset`, `cssAsset` |
...
| `usage.go` | Parse OpenAI- and Anthropic-compatible `usage` (including cache fields, Responses `response.usage`, embeddings `prompt_tokens`/`total_tokens`, and DashScope multimodal embedding `input_tokens` + top-level `image_tokens`; the tongyi `input_tokens_details` breakdown is never double-counted) from JSON/SSE |
