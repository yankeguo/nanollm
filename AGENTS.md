...
| `admin.go` / `admin_auth.go` / `admin_filter.go` / `admin_query.go` | Admin cookie login, usage dashboard, call viewer. Usage and Calls share GET filters: `range`/`from`/`to`/`tz`, `model`, `provider`, `api_key`, `outcome` (from `http_status`/`error`; the four outcomes partition `llm_calls`: ok / canceled / no_response / error). Usage windows are always bounded (custom max 366d); the chart bucket is derived from the span (≤48h hour, ≤62d day, else week), never user-selected. The usage chart series also carries per-bucket `AVG(NULLIF(first_token_ms, 0))` / `AVG(NULLIF(output_speed, 0))` (0 = no measurable calls, rendered as a line gap). Naive `from`/`to` are interpreted in `tz` (IANA, default UTC; `time/tzdata` is embedded). Call detail lists extracted files (`<img>` / `<video>` / `<audio>` / download) from `llm_call_files`. `/files/{sha}` serves bytes via `http.ServeContent` (Range support); stored MIME types are client-controlled, so only a safe inline allowlist is honored (`inlineFileMime`), everything else downloads as `application/octet-stream`, and `image/svg+xml` never renders inline. |
| `web/view/*.html` | Embedded HTML views for the admin dashboard (`/usage`, `/calls`, `/login`). Dark UI styled with Tailwind utilities plus the component classes from `main.css` (`.card`, `.btn*`, `.input`, `.tbl` with a sticky header, `.page-btn`, `.badge*`, the cache token bar, `.codebox`, `.stat-icon`, `.empty`, `.copybtn`/`.copy-inline`); icons are `<span class="icon-[lucide--NAME]"></span>` (Iconify masks, colored by `currentColor`) |
| `admin_tmpl.go` | `//go:embed web/view/*.html`; template funcs `formatNum`, `formatMs`, `formatSpeed`, `inputBar`, `statusClass`, `callErrorClass`, `jsAsset`, `cssAsset` |
